    ...
    var ShenBaoTocTree = {
        ...
        creatTocTree: function() {
            ...
            for (let i = 0; i < $tocTree.length; i++) {
                ...
                var item = {};
                item.tagName = (tag.prop("tagName")).toString().toLowerCase();
                item.id = tag.prop("id");
                item.title = tag.text();
            ...
            for (let j = 0; j < $tocTreeArray.length; j++) {
                var $item = '<li class="shenbao-toc-tree-li ' +
                ...
                if ($mark == 1) {
                    ...
                    $('.shenbao-toc-tree-hx').each(function() {
                        ...
                        if ($index >= $('.shenbao-toc-tree-hx').length - 1) {
                            ...
                            $divTop = $('.shenbao-toc-tree-hx').eq($index + 1).offset().top;
                        }
                        if ($divTop > $scrolltop) {
                            $('.shenbao-toc-tree-body li').removeClass('active');
                            $('.shenbao-toc-tree-body li').eq($index).addClass('active');
                    ...
                    var isTop = $scrolltop < $('.shenbao-toc-tree-hx').eq(0).offset().top;
                    var isBottom = $scrolltop > $('.the-end').offset().top;
                    if (isTop || isBottom) {
                        $('.shenbao-toc-tree-body li').removeClass('active');
                    }
                }
